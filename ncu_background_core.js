...
2. 对照上述参考标准答案，对每个小题分别进行评分 and 点评：
...
3. **【公式与计算式严苛批阅规则】**：若参考答案包含数学公式、物理公式或计算表达式（可能包含 LaTeX、物理符号、系数、指数等）：
   - 必须严密核对每一个**系数/常数**（如 \`3/4\`）、**物理量字母**（如 \`m\`, \`R\`, \`w\`/\`\\omega\`）和**指数幂**（如 \`^2\`）。
   - 若学生作答漏掉了系数、指数不对（如将 \`w^2\` 写成了 \`w\` 或将 \`R^2\` 写成了 \`R\`）、或者符号有任何遗漏，**绝对不能判定为 "correct"**。根据缺失严重程度，判定为 \`"partial"\`（部分正确并扣分）或 \`"incorrect"\`（完全错误）。
   - 细致甄别手写体容易混淆的字符（例如手写的 \`w\`、\`r\`、\`v\` 以及大小写 \`R\`/\`r\`）。特别注意区分相邻字母与上标的对应关系，不要发生错位结合（例如不要把手写的 \`w R^2\` 错看成 \`w^2\` 而漏掉了字母 \`R\`）。若图片中手写作答与标准答案不一致（例如学生只写了 \`mwR^2\`，而标准答案是 \`3/4*m*R^2*w^2\`），请按上述规则严格扣分。
4. 必须输出符合以下 JSON Schema 的纯 JSON 数据，不得包含任何 Markdown 格式包裹（不要使用 \`\`\`json 标记），不要包含任何额外的问候、思考过程或解释文字。
