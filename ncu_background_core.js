...
2. 进行【客观转录】：在评分前，你必须首先客观、原封不动地将图片中学生针对该小题的实际手写字迹转录为文本（填入 "studentAnswerOCR" 字段）。转录时**必须仅看图片，绝对禁止参考标准答案进行任何“脑补”或强行靠拢**。例如：若图片中学生手写分母清晰写为 \`2\`（如 \`3/2\`），你必须客观转录为 \`3/2\`，严禁转录为 \`3/4\`！若字母写得像 \`1\` 或是 \`l\`，必须客观转录为 \`1\` 或 \`l\`，严禁脑补为 \`R\`。**特别注意：在转录多条路径时，为防止因输出大量极度单调的重复符号和短连线触发 API 判定重复生成（Recitation）而强制截断输出，请勿使用圆圈数字（如①）或箭头符号（如->），而是将每条路径统一转录为用逗号分隔的普通数字格式，并为每条路径加上序号（例如：‘第1条独立路径：1, 13, 19；第2条独立路径：2, 3, 14, 19’），或者使用概括性文字转录（例如：‘学生列出了7条路径，其中6条以节点2开头走到19，1条以节点1开头’）。绝对禁止机械罗列含有大量箭头的长文本。**
...
4. **【填空题与公式严苛批阅规则】**：
   ...
   - **乘法交换律例外**：唯一的例外是**“因子可交换顺序”**。如果学生作答仅仅是乘积因子的书写顺序与参考答案不同（例如参考答案是 \`3/4*m*R^2*w^2\`，学生写为 \`m*w^2*R^2*3/4\` 等且数学/代数意义完全等价），**必须判定为 "correct"（正确，给满分）**。
...
`;
    ...
    if (apiProvider === "gemini") {
      ...
      if (apiProvider === "siliconflow" || apiProvider === "aliyun" || (apiUrl && (apiUrl.includes("siliconflow") || apiUrl.includes("aliyuncs.com") || apiUrl.includes("maas.aliyuncs.com")))) {
