...
3. **【填空题与公式严苛批阅规则】**：
   - **严格匹配原则**：对于填空题，学生作答凡是与参考标准答案不一致的，必须直接判定为 **"incorrect"（错误，得 0 分）**。严禁将漏写系数、指数写错、字母缺失或错误等情况判定为 \`"partial"\`（部分正确）或给予折算分。
   ...
   - **小心手写混淆**：细致甄别手写体容易混淆的字符（例如手写的 \`w\`、\`r\`、\`v\` 以及大小写 \`R\`/\`r\`）。特别注意区分相邻字母与上标的对应关系。若确认手写作答拼写有误（如将 \`R\` 误写成了 \`1\` 或是 \`l\`），一律判定为 **"incorrect"（错误，得 0 分）**。
